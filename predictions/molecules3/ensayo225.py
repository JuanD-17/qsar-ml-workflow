...
# 3. Calcular descriptores químicos y fingerprints Morgan (radius=3, fpSize=512) consistentes con el ensayo117, incluyendo descriptores de andamios de Murcko.
# 4. Preprocesar los descriptores (imputación y escalado) utilizando los objetos guardados en ensayo117.
...
# Modificación: Se asegura la compatibilidad con el modelo y descriptores del ensayo117. Se elimina la clasificación de potencia (Potency_Category). Se fuerza el uso de puntos como separador decimal al guardar el CSV para evitar problemas de formato regional.
# Categoría: Medio - rango superior medio - potencia
...
results_df.to_csv(os.path.join(output_dir, 'molecules3_predictions5.csv'), sep=';', index=False, float_format='%.6f')
logging.info(f"Predictions saved to {os.path.join(output_dir, 'molecules3_predictions5.csv')}")
