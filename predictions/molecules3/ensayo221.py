# Este script realiza un análisis de predicción de propiedades químicas (Log10 Value en nM, probablemente una medida de actividad biológica como IC50 o EC50) para moléculas de la familia Cannabaceae, utilizando un modelo de Random Forest previamente entrenado. El flujo de trabajo incluye:
...
# 3. Calcular descriptores químicos y fingerprints Morgan usando RDKit.
# 4. Preprocesar los descriptores (imputación y escalado).
# 5. Realizar predicciones con intervalos de confianza del 95%.
...
# Modificación: Se agrega la columna 'Molecule_name' al archivo de salida. Se fuerza el uso de puntos como separador decimal al guardar el CSV para evitar problemas de formato regional.
# El script está diseñado para ejecutarse en un entorno virtual de Python con las librerías necesarias instaladas (pandas, numpy, rdkit, sklearn, etc.).
# Categoría Bajo - Potencia Alta
import pandas as pd
import numpy as np
import os
import logging
...
tree_predictions = np.array([tree.predict(X_molecules_selected) for tree in rf_model.estimators_])
...
results_df.to_csv(os.path.join(output_dir, 'molecules3_predictions1.csv'), sep=';', index=False, float_format='%.6f')
logging.info(f"Predictions saved to {os.path.join(output_dir, 'molecules3_predictions1.csv')}")
