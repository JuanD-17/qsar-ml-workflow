# Este script realiza un análisis de predicción de propiedades químicas (Log10 Value en nM, probablemente una medida de actividad antioxidante como IC50 o EC50) para moléculas de la familia Cannabaceae, utilizando un modelo de Stacking (Random Forest + MLP con meta-modelo de regresión lineal) previamente entrenado en el ensayo171 con selección de características Boruta, Mutual Information, y descriptores con andamios de Murcko. El flujo de trabajo incluye:
...
# 3. Calcular descriptores químicos y fingerprints Morgan (radius=4, fpSize=512) consistentes con el ensayo171, incluyendo descriptores de andamios de Murcko.
# 4. Preprocesar los descriptores (imputación y escalado) utilizando los objetos guardados en ensayo171.
# 5. Realizar predicciones con intervalos de confianza del 95% usando el modelo de Stacking, con intervalos basados en los estimadores de Random Forest.
...
# Modificación: Se asegura la compatibilidad con el modelo y descriptores del ensayo171. No se incluye clasificación de potencia. Se fuerza el uso de puntos como separador decimal al guardar el CSV para evitar problemas de formato regional.
# Categoría: Alto
...
try:
    stacking_model = joblib.load(os.path.join(input_dir, 'stacking_model_Alto.joblib'))
    scaler = joblib.load(os.path.join(input_dir, 'scaler_Alto.joblib'))
    imputer = joblib.load(os.path.join(input_dir, 'imputer_Alto.joblib'))
    selected_features = joblib.load(os.path.join(input_dir, 'selected_features_Alto.joblib'))
...
try:
    df_molecules = pd.read_csv('molecules3.csv', sep=';')
    logging.info(f"Initial rows in molecules3.csv: {len(df_molecules)}")
except Exception as e:
    logging.error(f"Error loading molecules3.csv: {e}")
    exit()

# Verifica columnas 'Smiles' y 'Molecule_name'
smiles_column = 'Smiles'
molecule_name_column = 'Molecule_name'
df_molecules.columns = df_molecules.columns.str.strip()
if smiles_column not in df_molecules.columns:
    logging.error(f"La columna '{smiles_column}' no se encuentra en el archivo CSV.")
    logging.info(f"Columnas disponibles: {df_molecules.columns.tolist()}")
    exit()
if molecule_name_column not in df_molecules.columns:
    logging.error(f"La columna '{molecule_name_column}' no se encuentra en el archivo CSV.")
    logging.info(f"Columnas disponibles: {df_molecules.columns.tolist()}")
    exit()

# Valida las SMILES
logging.info("Validating SMILES for molecules...")
smiles_list, valid_idx = validate_smiles(df_molecules[smiles_column].tolist())
logging.info(f"Valid SMILES: {len(smiles_list)}")
if not smiles_list:
    logging.error("No valid SMILES found in molecules3.csv.")
    exit()

# Obtiene los nombres de moléculas correspondientes
molecule_names = [df_molecules[molecule_name_column].iloc[i] for i in valid_idx]

# Calcula descriptores
logging.info(f"Calculating descriptors for {len(smiles_list)} valid SMILES...")
X_molecules, valid_idx = get_descriptors(smiles_list)
logging.info(f"Descriptors shape: {X_molecules.shape}")

# Preprocesa los descriptores
X_molecules = imputer.transform(X_molecules)
X_molecules_scaled = scaler.transform(X_molecules)
X_molecules_selected = X_molecules_scaled[:, selected_features]
logging.info(f"Selected features shape: {X_molecules_selected.shape}")
...
# Usa solo el estimador Random Forest del stacking para calcular intervalos de confianza
rf_estimator = stacking_model.named_estimators_['rf']
tree_predictions = np.array([tree.predict(X_molecules_selected) for tree in rf_estimator.estimators_])
mean_predictions = stacking_model.predict(X_molecules_selected)  # Predicciones del modelo de stacking
std_predictions = np.std(tree_predictions, axis=0)
ci_margin = z_score * std_predictions / np.sqrt(len(rf_estimator.estimators_))
...
results_df.to_csv(os.path.join(output_dir, 'molecules3_predictions.csv'), sep=';', index=False, float_format='%.6f')
logging.info(f"Predictions saved to {os.path.join(output_dir, 'molecules3_predictions.csv')}")
