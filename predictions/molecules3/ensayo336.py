...
# 1. CARGA CONSOLIDADA: Carga molecules3.csv y las predicciones de TODOS los modelos de los análisis anteriores (Bajo y Medio), asegurando una fila única por molécula.
...
MODEL_MAPPING = {
    ...
    "Bajo_1": ("ensayo221", "molecules3_predictions1.csv", "Bajo"),
    "Bajo_2": ("ensayo222", "molecules3_predictions2.csv", "Bajo"),
    "Bajo_3": ("ensayo223", "molecules3_predictions3.csv", "Bajo"),
    # Modelos de Categoría MEDIO
    "Medio_1": ("ensayo224", "molecules3_predictions4.csv", "Medio"),
    "Medio_2": ("ensayo225", "molecules3_predictions5.csv", "Medio"),
    "Medio_3": ("ensayo226", "molecules3_predictions6.csv", "Medio"),
    "Medio_4": ("ensayo227", "molecules3_predictions7.csv", "Medio"),
    # Modelos de Categoría ALTO (integrados ensayo171)
    "Alto_1": ("ensayo228", "molecules3_predictions8.csv", "Alto"),
}
...
try:
    df_base = pd.read_csv('molecules3.csv', sep=';')
    df_base = df_base.drop_duplicates(subset=['Smiles']).reset_index(drop=True)
    df_base.columns = df_base.columns.str.strip()
except Exception as e:
    logging.error(f"Error al cargar molecules3.csv: {e}")
    exit()
...
output_path = os.path.join(output_dir, "molecules3_consolidated_predictions.csv")
output_df.to_csv(output_path, sep=';', index=False, float_format='%.6f')

output_xlsx_path = os.path.join(output_dir, "molecules3_consolidated_predictions.xlsx")
