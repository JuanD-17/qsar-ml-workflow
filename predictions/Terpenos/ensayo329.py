...
# Carga las moléculas de Terpenos
logging.info("Loading Terpenos molecules...")
try:
    df_cannabaceae = pd.read_csv('Terpenos.csv', sep=';')
    logging.info(f"Initial rows in Terpenos.csv: {len(df_cannabaceae)}")
except Exception as e:
    logging.error(f"Error loading Terpenos.csv: {e}")
    exit()
...
tree_predictions = np.array([tree.predict(X_cannabaceae_selected) for tree in rf_estimator.estimators_])
mean_predictions = stacking_model.predict(X_cannabaceae_selected)  # Predicciones del modelo de stacking
...
results_df.to_csv(os.path.join(output_dir, 'terpenos_predictions8.csv'), sep=';', index=False, float_format='%.6f')
logging.info(f"Predictions saved to {os.path.join(output_dir, 'terpenos_predictions8.csv')}")
