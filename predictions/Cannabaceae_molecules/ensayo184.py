...
try:
    df_cannabaceae = pd.read_csv('Cannabaceae_molecules.csv', sep=';')
    logging.info(f"Initial rows in Cannabaceae_molecules.csv: {len(df_cannabaceae)}")
except Exception as e:
    logging.error(f"Error loading Cannabaceae_molecules.csv: {e}")
    exit()
...
if not smiles_list:
    logging.error("No valid SMILES found in Cannabaceae_molecules.csv.")
    exit()
...
results_df.to_csv(os.path.join(output_dir, 'cannabaceae_predictions8.csv'), sep=';', index=False, float_format='%.6f')
logging.info(f"Predictions saved to {os.path.join(output_dir, 'cannabaceae_predictions8.csv')}")
